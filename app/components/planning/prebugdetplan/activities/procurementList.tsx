...
const ProcurementList: React.FC<ActivitiesProps> =({isHidden})=>{
    ...
    return (
        ...
        <table className="w-full table-auto ms-2">
          <thead>
            <tr className= {!isHidden? "text-xs  border-t-2  border-b-2":"text-xs  border-t-2  border-r-2 border-b-2"}>
              ...
              <div onClick={updateAllCheckBox}>
                       ...
                       <span className="flex border border-black w-4 ps-1  pt-1 pb-2 pr-1">
              ...
              <th className="text-left align-top w-24 ">
                Procurement Method
              </th>
          ...
          <tbody>
            {dataList.map((activity) => {
              return (
                <tr
                  className= {!isHidden? "text-xs  border-b-2" : "text-xs  border-r-2 border-b-2"}
                  key={activity.id}
                >
                  <td className="text-left ps-2">
                    <label>
                      <div onClick={()=>{updateCheckBox(activity.id)}}>
                        {activity.isChecked ? (
                          <>
          <span className="flex border border-black w-4">
            <Image src="/checked.png" alt="" width={10} height={10} />
                  ...
                  <td className="text-left ps-1 align-bottom">
                    {activity.name}
                  </td>
                  { !isHidden &&
              <>
              ...
              <td>
                <span className="flex gap-1">
                  <span
                    className="flex  border w-16 border-black rounded-sm pt-1 pb-1 pr-1
